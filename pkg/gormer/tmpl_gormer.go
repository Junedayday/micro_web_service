...
func ({{.StructName.LowerS}} *{{.StructName.UpperS}}) GetValueByField(field {{.StructName.UpperS}}Field) interface{} {
    switch field {
{{range $item := .Columns}}
        case {{$.StructName.UpperS}}Field{{$item.FieldName}}:
			return {{$.StructName.LowerS}}.{{$item.FieldName}} {{end}}
	}
	return 0
}
